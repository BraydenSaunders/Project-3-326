...
public class Sudoku implements CSPProblem<Square,Integer> {
    ...
    public Map<Square,Variable<Square,Integer>> getAllVariables() {
        ...
        try {
            ...
            for (int i=0; i<9; i++) {
                ...
                //if a line is empty, or if the remaining lines are missing, then treat an empty or missing line as all squares in that line are open
                    for (int j=0; j<9; j++) {
                        Square vName = new Square(i, j);
                        Variable<Square,Integer> v = new Variable<>(vName, new LinkedList<>(defaultDomain));
                        allVariables.put(vName,v);
                    }
                }
            }
        ...
        for (int i=0; i<9; i++) {
            for (int j=0; j<9; j++) {
                if (allVariables.get(new Square(i,j)).domain().size() > 1) {
                    ...
                    System.out.print("["+allVariables.get(new
                            Square(i,j)).domain().getFirst()+"]");
                }
            }
            System.out.print("\r\n");
        }
    ...
    //TODO:
        ...
        return new LinkedList<>(neighbors.get(sq));
    }
    /**
...
//TODO:
        ...
        //go through allVariables
        //if a variable's domain size is 1, add the variable to the list
        //return the list at the end
        List<Square> list = new LinkedList<>();
        for (Variable<Square, Integer> variable : allVariables.values()){
